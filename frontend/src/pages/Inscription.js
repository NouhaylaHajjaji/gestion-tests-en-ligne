...
const Inscription = () => {
  ...
  const fetchCreneaux = async () => {
    try {
      const response = await fetch('http://localhost:8080/gestion-tests-backend/api/creneaux/disponibles');
      if (response.ok) {
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('http://localhost:8080/gestion-tests-backend/api/candidats/inscription', {
        method: 'POST',
        headers: {
