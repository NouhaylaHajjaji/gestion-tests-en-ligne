...
const ScheduleManagement = () => {
  ...
  const fetchCreneaux = async () => {
    try {
      const response = await fetch('http://localhost:8080/gestion-tests-backend/api/admin/creneaux');
      if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingCreneau 
        ? `http://localhost:8080/gestion-tests-backend/api/admin/creneaux/${editingCreneau.id}`
        : 'http://localhost:8080/gestion-tests-backend/api/admin/creneaux';
  ...
  const handleDelete = async (creneauId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/gestion-tests-backend/api/admin/creneaux/${creneauId}`, {
        method: 'DELETE'
      });
